...
"""NPU correctness check: TP0 GVA pack + batched H2D + sparse_copy D2D scatter."""
...
def test_packed_host_gather_contiguous_h2d_and_d2d():
    ...
    try:
        ...
        scatter_packed = torch.zeros(num_entries, dtype=torch.int64)
        assert cpp.packed_fill_scatter_srcs(
            src,
            dst,
            sizes,
            num_entries,
        ...
        packed_base = int(packed_host.data_ptr())
        for i in range(num_entries):
            assert int(scatter_packed[i].item()) == packed_base + i * entry_bytes
        ...
        scatter_src = torch.zeros(num_entries, dtype=torch.int64)
        assert cpp.packed_fill_scatter_srcs(
            src,
            dst,
            sizes,
        ...
        src_npu = scatter_src.to(device)
        dst_npu = dst.to(device)
        size_npu = sizes.to(device)
        num_npu = torch.tensor([num_entries], dtype=torch.int32, device=device)
        assert offload.sparse_copy(src_npu, dst_npu, size_npu, num_npu, device) in (None, 0)
