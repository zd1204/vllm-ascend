...
def run_same_process() -> None:
    ...
    try:
        ...
        cpp.packed_fill_scatter_srcs(src, dst0, sizes, NUM_ENTRIES, int(staging0.data_ptr()), TOTAL_BYTES, scatter_src)
        offload.sparse_copy(
            scatter_src.to(device),
            dst0.to(device),
            sizes.to(device),
            torch.tensor([NUM_ENTRIES], dtype=torch.int32, device=device),
            device,
        )
        ...
        scatter_src = torch.zeros(NUM_ENTRIES, dtype=torch.int64)
        cpp.packed_fill_scatter_srcs(src, dst1, sizes, NUM_ENTRIES, int(staging1.data_ptr()), TOTAL_BYTES, scatter_src)
        offload.sparse_copy(
            scatter_src.to(device),
            dst1.to(device),
            sizes.to(device),
